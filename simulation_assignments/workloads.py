    ...
    with gzip.open(path, 'rt', newline='') as f:
        ...
        for row in csv.DictReader(f):
            ...
            try:
                ...
                continue
            jobs.append((submit, start, end, int(row['node_count'])))
            
        jobs.sort(key=lambda x: x[0])  # sort by submit_time
        
        result = []
        last_submit = None
        for submit, start, end, node_count in jobs:
            delay = submit - last_submit if last_submit is not None else 0
            assert delay >= 0
            result.append((delay, (end - start) * node_count))
            last_submit = submit
...
def normalize_trace(trace, lambd, mu=1):
    ...
    for delay, size in trace:
        delay_sum += delay
        size_sum += size
    delay_factor = n / (delay_sum * lambd) if delay_sum > 0 else 0
    size_factor = n / (size_sum * mu) if size_sum > 0 else 0
